...
export function LoadsPage({ orderId, embedded }: { orderId?: string; embedded?: boolean }) {
  ...
  const list = useLoads({ q: q || undefined, orderId, status: current.statuses, pageSize: 200 });
  const all = useLoads({ orderId, pageSize: 200 });
  const counts = Object.fromEntries(TABS.map((t) => [t.key, t.statuses ? (all.data?.items ?? []).filter((l) => t.statuses!.includes(l.status)).length : (all.data?.total ?? 0)]));
  ...
  const body = (
    <Card className="overflow-hidden">
      ...
      {list.isLoading ? (
        ...
        <EmptyState icon={<Truck />} title="Nenhuma carga" description={embedded ? 'As cargas surgem ao converter agendamentos ou ao registrar uma carga direta.' : 'Ajuste a etapa ou a busca.'} />
      ) : (
              ...
              {items.map((l: LoadDto) => (
                <tr key={l.id} tabIndex={0} onClick={() => setOpenId(l.id)} onKeyDown={(e) => e.key === 'Enter' && setOpenId(l.id)} className="cursor-pointer border-b border-border/60 outline-none hover:bg-primary-soft/30 focus-visible:bg-primary-soft/40">
                  <td className="h-14 px-4">
                    ...
                    <div className="text-[11px] text-subtle">{l.loadingDate ? formatDate(l.loadingDate) : 'sem data'}</div>
                  </td>
                  {!orderId ? (
        ...
        <div className="mx-auto flex max-w-[1800px] flex-col gap-5 px-4 py-6 sm:px-6 lg:px-8">
          <div className="flex items-center gap-3.5">
            ...
            <div>
              ...
              <p className="text-sm text-muted">Carregamentos físicos de cada ordem: frota, pesagem, transporte e recebimento.</p>
            </div>
          </div>
